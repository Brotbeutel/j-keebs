...
- [x] ~~Verification scripts in the repo root~~ — moved to `scripts/` by the owner (`2749621`) and documented in `CONVENTIONS.md` "Verification" (planner, 2026-09-21). Still open: `check_links.py` has `ORIGIN`/`BASE` hard-coded to the project page; a CI step that runs it is part of P2-F. Run: `python scripts/check_links.py _site`, `python scripts/tag_balance.py _site` (Python 3, standard library only; `tag_balance.py` needs the folder argument).

## Found during planner review 2026-09-21 (`2749621`)

- [ ] **Low — stray file `i` in the repo root.** A curl cookie-jar file (`# Netscape HTTP Cookie File`, 135 bytes), probably from a `curl -c i` typo. Owner step: `git rm i`.
- [ ] **Low — `ai/AGENTS.md` duplicates the root `AGENTS.md`** (byte-identical, added in `e45aa12`). Two copies drift apart. Keep the root one (agent tools look there; `ai/README.md` and `readme.md` only list the root file), `git rm ai/AGENTS.md`.
- [x] ~~**Med — Node ≥ 22 becomes a requirement with P2-E** (`@11ty/eleventy-img` 7.x). `readme.md` currently says "Node.js 18 or newer (CI uses 22)"; update it in P2-E.~~ Done 2026-09-23: `readme.md` and `ai/CONVENTIONS.md` now say Node ≥ 22.
- [ ] **Info — gallery slides are stacked with `opacity`, so every slide of a card loads once the card is near the viewport** (lazy loading ignores opacity). Harmless with 40–200 KB files after P2-E; a slide-on-demand loader would only be worth it if the numbers say so.
- [ ] **Info — `.git` is ~103 MB** (history of the removed `node_modules/` and originals). Not a problem for Pages; rewriting history is not recommended.
- [ ] **Owner decision still open:** R6 — which German gallery title is final ("Der Garagenfund" in the HTML vs "Cherry G80-3000" in the DE dictionary), then EN follows.
...
- [x] ~~Image `srcset` / WebP~~ → R3 / **P2-E, implemented 2026-09-23** (sandbox, not in `main` yet). (`index.html` `og:image` exists now; format problem → R9.)
...
- [x] No undisclosed third-party requests (R1) — fonts self-hosted, done in P2-D (`38efe4f`); owner reported all checks positive 2026-09-21. (Remaining external hosts: OpenStreetMap iframe, FormSubmit — both disclosed.)

## Done

- 2026-09-23 — Implementer: P2-E built in the sandbox (not yet copied into the repo / deployed) — build-time image pipeline (`imagePipeline` transform in `eleventy.config.js`, `@11ty/eleventy-img` 7.0.0, Node ≥ 22 now required). WebP `srcset`/`sizes`/`width`/`height`/`data-full` on every processed `<img>`, templates untouched. R3 and the rest of R4 (wrong `width`/`height` attributes) fixed. `main.js` fullscreen fix (1 line). Full report, numbers and `sizes` derivation: `ai/STATUS.md` "P2-E". Changed: `eleventy.config.js`, `main.js`, `.gitignore`, `readme.md`, `ai/CONVENTIONS.md`, `package.json`/`package-lock.json` (new dependency); new: `scripts/img_report.py`. Unchanged: every `src/pages/*.njk`, `src/_includes/base.njk`, `style.css`.
- 2026-09-21 — Planner: P2-D re-verified on `2749621` (build, `scripts/check_links.py` 0 errors, all P2-D items confirmed); scripts folder documented; P2-E planned after a feasibility spike (transform plugin does not resolve root-level `images/`; `Image()` API measured); P2-E2 added to the sequence.
- 2026-09-21 — P2-D done, committed as `38efe4f` and checked by the owner (all checks positive): self-hosted fonts (`fonts/`, 10 woff2 + 4 OFL licences, `@font-face` in `style.css`, Google `<link>` tags removed from `base.njk`); logo `width/height` 1742×733 (header + footer); 404 without `<base>` (`root_paths` in `base.njk`, set in `404.njk`); PCBWay logo link `target`/`rel`; `node_modules/` and `.cursor/` untracked (R2). Open from that review: R3, R6, R7, `Werkbank_Hero.jpg` attributes (rest of R4).
