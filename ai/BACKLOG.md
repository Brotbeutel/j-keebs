...
- [ ] **Low — verification scripts in the repo root.** `check_links.py` and `tag_balance.py` (written for the P2-D checks) were committed to the repo root in `38efe4f`. Either move them to a `scripts/` folder and document them in `CONVENTIONS.md` "Verification" (they also cover the "CI: link/asset check" idea), or remove them. `check_links.py` has `ORIGIN`/`BASE` hard-coded to the project page. Run: `python check_links.py _site`, `python tag_balance.py _site` (Python 3, standard library only).
