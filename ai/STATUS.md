...
- Google Fonts are loaded from Google servers on every page; `privacy.njk` / `cookies.njk` do not mention it (R1).
...
- Logo `width`/`height` attributes in `base.njk` are wrong (1742×980, real 1742×733) → header layout shift (R4).
- `404.njk` puts `<base>` after the stylesheet link; skip-link breaks on the 404 page (R5).
...
- [ ] **P2-D — repo hygiene, self-hosted fonts, base-layout fixes** ← current package (see `PLAN.md`)
- [ ] P2-E — image pipeline
...
| Fonts | Google Fonts `<link>` in `base.njk` (external request — see R1) |
...
| Fonts | Self-hosted woff2, no third-party font requests — P2-D |
