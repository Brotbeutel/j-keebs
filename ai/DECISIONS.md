...
| 2026-09-05 | Contact-page OpenStreetMap embed loads automatically on page load, not click-to-load | Explicit owner instruction, overriding the click-to-load pattern implemented the same day. Owner also wants it desaturated by default, full color on hover/focus (`.map-embed__frame iframe`, `filter: grayscale`). **This is a deliberate reversal, not an oversight** — do not "fix" it back to click-to-load. Consequence: every visit to contact.html now sends the visitor's IP to OpenStreetMap automatically; `cookies.html` and `privacy.html#kartendienst` were updated the same day to describe this honestly (legal basis recorded as Art. 6 Abs. 1 lit. f DSGVO, berechtigtes Interesse — not lit. a/consent, since there's no user action anymore). Flagged to the owner in chat as a privacy-posture change worth a legal gut-check; not verified by a lawyer. |
| 2026-09-09 | Eleventy is the approved next architecture for the static site; Astro remains a possible later migration | Shared HTML chrome is duplicated across the site, while the current product still benefits from plain static output and simple GitHub Pages hosting. Eleventy can centralize layouts, data, metadata, and Markdown without requiring a client framework. The migration must preserve the `/j-keebs/` project-page base path, existing canonical URLs, redirect stubs, visual behavior, and owner-editable content. Do not start an Astro or React rewrite as part of the Eleventy work. |
| 2026-09-11 | Eleventy templates in `src/pages/` are the single source of truth; GitHub Pages requires `_site/` deployment | P2-C migrated all 21 canonical pages to `src/pages/*.njk` and removed the legacy root `.html` files. GitHub Pages must serve from the `_site/` build output (via GitHub Actions workflow using `actions/deploy-pages` or a `gh-pages` branch) rather than the repository root. |
